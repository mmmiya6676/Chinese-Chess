...
InputResult mouseInputLoop(Game& game) {
    ...
    // 基于当前模式：加鼠标输入，去掉行缓冲和回显（否则鼠标事件被回车阻塞）
    // 去掉快速编辑（否则左键被系统拦截去选文本）
    ...
    while (true) {
        ...
        if (numEvents > 0) {
            ...
            if (needTextInput) {
                ...
                cout << "--- 键盘输入模式 (ESC取消) ---" << flush;
                COORD hint2 = {0, 18};
                SetConsoleCursorPosition(hOut, hint2);
                cout << "坐标(x1 y1 x2 y2) | 记谱法(炮二平五) | U悔棋 S保存 Q退出" << flush;
                // 输入行
                COORD promptPos = {0, 19};
                SetConsoleCursorPosition(hOut, promptPos);
                cout << "> " << string(60, ' ') << flush;
                SetConsoleCursorPosition(hOut, {2, 19});
                cout << flush;

                string kbInput;
                while (true) {
                    DWORD n2;
                    GetNumberOfConsoleInputEvents(hIn, &n2);
                    if (n2 > 0) {
                        INPUT_RECORD ir2;
                        DWORD rd;
                        ReadConsoleInput(hIn, &ir2, 1, &rd);
                        if (ir2.EventType == KEY_EVENT && ir2.Event.KeyEvent.bKeyDown) {
                            char ch = ir2.Event.KeyEvent.uChar.AsciiChar;
                            WORD vk2 = ir2.Event.KeyEvent.wVirtualKeyCode;
                            if (ch == '\r' || ch == '\n') break;
                            if (vk2 == VK_ESCAPE) { kbInput.clear(); break; }
                            if (ch == '\b') { if (!kbInput.empty()) kbInput.pop_back(); }
                            else if (ch >= ' ') kbInput += ch;
                            COORD pp = {2, 19};
